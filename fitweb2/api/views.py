...
class PlaceViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        ...
        if self.action == "list" and not is_admin_user:
            return PlaceSummarySerializer
...
class SessionSlotViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        place_id = self.request.query_params.get("place") or self.request.query_params.get("place_id")
...
class PlaceReviewViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        is_admin = self.request.user.is_staff or getattr(self.request.user, "is_admin", False)
        if not is_admin:
            qs = qs.filter(place__is_active=True)
        if self.action in {"update", "partial_update", "destroy"} and not is_admin:
...
class TrainerReviewViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action == "create":
            return [permissions.IsAuthenticated()]
        if self.action in {"update", "partial_update", "destroy", "toggle_visible"}:
            return [permissions.IsAdminUser()]
        return [permissions.AllowAny()]

    def get_queryset(self):
        trainer_id = self.request.query_params.get("trainer")
        qs = TrainerReview.objects.select_related(
            "trainer",
            "user",
            "booking",
            "booking__slot",
    ...
    def toggle_visible(self, request, pk=None):
        ...
        return Response({"is_visible": review.is_visible})
