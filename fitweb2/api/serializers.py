...
from reviews.models import Review as PlaceReview, TrainerReview
...
class BookingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Booking
        fields = ["id", "user", "slot", "slot_id", "status", "created_at", "updated_at"]
...
class TrainerReviewSerializer(serializers.ModelSerializer):
    ...
    booking_id = serializers.PrimaryKeyRelatedField(
        source="booking",
        queryset=Booking.objects.select_related("slot__trainer"),
        write_only=True,
        required=True,
    )
    ...
    class Meta:
        ...
        read_only_fields = ["id", "user", "trainer", "is_visible", "created_at"]

    def validate(self, attrs: dict[str, Any]) -> dict[str, Any]:
        request = self.context.get("request")
        user = getattr(request, "user", None)
        booking: Booking = attrs["booking"]
        trainer: Trainer = attrs["trainer"]

        if booking.user_id != getattr(user, "id", None):
            raise serializers.ValidationError("You can only review your own booking.")
        if booking.status != Booking.Status.COMPLETED:
            raise serializers.ValidationError("Booking must be completed before reviewing.")
        if booking.slot.trainer_id != trainer.id:
            raise serializers.ValidationError("Trainer does not match the booking.")
        if TrainerReview.objects.filter(booking=booking).exists():
    ...
    def create(self, validated_data: dict[str, Any]) -> TrainerReview:
        request = self.context.get("request")
        user = getattr(request, "user", None)
        return TrainerReview.objects.create(user=user, **validated_data)
