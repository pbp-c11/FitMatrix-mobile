...
def _persist_secret_key(key_file: Path, secret_key: str) -> str:
    """Write the key once (no overwrite) and return the stable value."""
    try:
        ...
        try:
            ...
            return existing_key or secret_key
        except OSError:
            return secret_key
    except OSError:
        return secret_key
...
def _load_secret_key() -> str:
    ...
    env_key = (os.getenv("DJANGO_SECRET_KEY") or os.getenv("SECRET_KEY") or "").strip()
    if env_key:
        return env_key

    key_file_env = os.getenv("DJANGO_SECRET_KEY_FILE")
    key_file = Path(key_file_env) if key_file_env else (BASE_DIR / ".django_secret_key")
    try:
    ...
    seed = (os.getenv("DB_PASSWORD") or os.getenv("DATABASE_URL") or "").strip()
    if seed:
        material = "|".join(
            [
                seed,
        ...
        derived_key = hashlib.sha256(material.encode("utf-8")).hexdigest()
        return _persist_secret_key(key_file, derived_key)

    if PRODUCTION:
        raise ImproperlyConfigured(
            "DJANGO_SECRET_KEY (or SECRET_KEY) must be set when PRODUCTION=true"
        )

    generated_key = get_random_secret_key()
    return _persist_secret_key(key_file, generated_key)


SECRET_KEY = _load_secret_key()
DEBUG = os.getenv("DJANGO_DEBUG", "true" if not PRODUCTION else "false").lower() == "true"
...
# Serve from STATICFILES_DIRS/app static without requiring collectstatic.
