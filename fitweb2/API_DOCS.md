...
## Admin
- `GET admin/summary/` (admin) ƒ?" dashboard counts (users/bookings/places/trainers/upcoming slots).
- `GET admin/admins/` (admin) ƒ?" list admin accounts.
- `POST admin/admins/` (admin) ƒ?" create admin (`username`, `email`, `password`, optional `display_name`).
...
## Session slots
- `GET sessions/` — query: `q`, `trainer`, `place`; returns slots with trainer/place + `seats_left`.
...
- `POST trainer-reviews/` — body: `trainer_id`, `booking_id`, `rating`, `comment` (auth, booking owner, booking must be completed & unused).
- `POST trainer-reviews/{id}/toggle_visible/` (admin) — toggle review visibility.
