...
def trainer_list(request: HttpRequest) -> HttpResponse:
    ...
    trainers = trainers.annotate(
        next_available=Min(
            "slots__start",
            filter=Q(
                slots__start__gte=timezone.now(),
                slots__is_active=True,
            ),
        ),
        active_slots=Count(
            "slots",
...
def trainer_detail(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    upcoming_slots = (
        trainer.slots
        .filter(is_active=True, start__gte=timezone.now())
    ...
    is_favorite = False
    if request.user.is_authenticated:
        is_favorite = WishlistItem.objects.filter(
            user=request.user,
            trainer=trainer,
        ).exists()

    reviews = (
        TrainerReview.objects
        .filter(trainer=trainer, is_visible=True)
        .select_related("user")
        .order_by("-created_at")
    )
...
def booking_create(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    if slot.seats_left() <= 0:
        messages.error(request, "This session is already fully booked.")
        return redirect("scheduling:trainer-detail", pk=slot.trainer_id)
    ...
    messages.success(
        request,
        f"Session with {booking.slot.trainer.name} on "
        f"{timezone.localtime(booking.slot.start):%d %b %H:%M} confirmed."
    )
...
def booking_manage(request: HttpRequest) -> HttpResponse:
    ...
    upcoming = [
        b for b in bookings
        if b.status == Booking.Status.BOOKED and b.slot.start >= now
    ]

    past = [
        b for b in bookings
        if b.slot.start < now or b.status == Booking.Status.CANCELLED
    ]
...
def booking_reschedule(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            booking.slot = form.cleaned_data["new_slot"]
...
def upcoming_sessions(request: HttpRequest) -> HttpResponse:
    ...
    slots_qs = SessionSlot.objects.select_related(
        "trainer",
        "place",
    ).filter(
        is_active=True,
